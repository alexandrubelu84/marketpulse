export default async function handler(req, res) {
  ...
  const instruments = [
    ...
    { label: "German Gas", symbol: "TTFDE=F",unit: "EUR/MWh" },
  ...
  try {
    ...
    // Try v7 quote
    const url2 = `https://query2.finance.yahoo.com/v7/finance/quote?symbols=${encodeURIComponent(tickers)}`;
    const r2 = await fetch(url2, {
      headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)", "Accept": "application/json" }
    });

    if (!r2.ok) throw new Error("Yahoo v7 " + r2.status);
    ...
    try {
      ...
      const url3 = `https://query1.finance.yahoo.com/v7/finance/quote?symbols=${tickers}`;
